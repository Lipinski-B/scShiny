...
CD79B_Y196H <- got(read.table("CD79B_Y196H.out/CD79B_Y196H.summTable.txt", header = T), "CD79B_Y696H", patient)
EZH2_A682G <- got(read.table("EZH2_A682G.out/EZH2_A682G.summTable.txt", header = T), "EZH2_A682G_2", patient)
EZH2_A692V <- got(read.table("EZH2_A692V.out/EZH2_A692V.summTable.txt", header = T), "EZH2_A692V_1", patient)
...
GOT<- cbind(BCL2_L23L[["tomerge"]], BCL2_K22K[["tomerge"]], CD79B_Y196H[["tomerge"]], EZH2_A682G[["tomerge"]], EZH2_A692V[["tomerge"]], EZH2_Y646C[["tomerge"]],EZH2_Y646F[["tomerge"]], EZH2_Y646H[["tomerge"]], EZH2_Y646N[["tomerge"]], EZH2_Y646S[["tomerge"]])
rownames(GOT) <-paste0(rownames(GOT),"-1")
colnames(GOT) <- c( "BCL2_L23L","BCL2_K22K", "CD79B_Y196H","EZH2_A682G","EZH2_A692V","EZH2_Y646C","EZH2_Y646F","EZH2_Y646H","EZH2_Y646N","EZH2_Y646S")
save(GOT, file=paste0("/home/boris/Documents/lipinskib/flinovo/result/", patient, "/R/", patient, "_GoT.Rdata"))

##################################################################################################################################################################
######### Violin plot #########
###############################
#cd79B <- vlnp("CD79B","CD79B_Y196H",patient)
bcl2 <- vlnp("BCL2", "BCL2_K22K",patient)
ezh2 <- vlnp("EZH2","EZH2_A682G",patient)

a <- rbind(ezh2, bcl2)

png(filename = paste0("/home/boris/Documents/lipinskib/flinovo/result/", patient, "/GOT/VLP.png"))
...
histo <- data.frame(hotspot=c(rep("BCL2_L23L",length(table(useNA = "always", BCL2_L23L[[1]]))),rep("BCL2_K22K",length(table(useNA = "always", BCL2_K22K[[1]]))),
                              rep("CD79B_Y196H",length(table(useNA = "always", CD79B_Y196H[[1]]))),rep("EZH2_A682G",length(table(useNA = "always", EZH2_A682G[[1]]))),
                              rep("EZH2_A692V",length(table(useNA = "always", EZH2_A692V[[1]]))),
                    ...
                    Génotype=c(names(table(useNA = "always", BCL2_L23L[[1]])),names(table(useNA = "always", BCL2_K22K[[1]])),names(table(useNA = "always", CD79B_Y196H[[1]])),names(table(useNA = "always", EZH2_A682G[[1]])),
                               names(table(useNA = "always", EZH2_A692V[[1]])), names(table(useNA = "always", EZH2_Y646C[[1]])),
...
png(filename = paste0("/home/boris/Documents/lipinskib/flinovo/result/", patient, "/GOT/histo.png"))
