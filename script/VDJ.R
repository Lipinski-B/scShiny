...
for (patient in c("FL140304","FL12C1888","FL09C1164","FL08G0293")) {
  ...
  save(Clonotype, Type, Isotype, V, D, J, vloupe, Heavy, Light, file=paste0("/home/boris/Documents/lipinskib/flinovo/result/",patient,"/R/VDJ.RData"))

  load(file = paste0("/home/boris/Documents/analyse/singlet_", patient,".RData"))
  
  all@tools$Clonotype <- Clonotype
  all@tools$Type <- Type
  all@tools$Isotype <- Isotype
  all@tools$V <- V
  all@tools$D <- D
  all@tools$J <- J
  all@tools$vloupe <- vloupe
  all@tools$Light <- Light
  all@tools$Heavy <- Heavy
